...
public class DirectoryConnector {
	...
	private byte[] sendAndReceiveDatagrams(byte[] requestData) {
		 ...
		 */
				try {
					...
					response = new byte[packetToReceive.getLength()];
					System.arraycopy(packetToReceive.getData(), packetToReceive.getOffset(), response, 0, packetToReceive.getLength());

				} catch (SocketTimeoutException e) {
					attempts++;
					System.err.println("Timeout expirado. Reintento " + attempts + " de " + MAX_NUMBER_OF_ATTEMPTS);
				}
			}

			if (!received) {
				System.err.println("Error: No se obtuvo respuesta tras " + MAX_NUMBER_OF_ATTEMPTS + " intentos.");
			}

		} catch (IOException e) {
			System.err.println("Error crítico de entrada/salida (E/S) en el socket UDP.");
			e.printStackTrace();
			System.exit(-1);
		}
	...
	public boolean pingDirectoryRaw() {
		...
		if (responseData != null) {
			...
			if (responseString.equals("welcome")) {
				//System.out.println("Conexión con el directorio establecida (welcome).");
				} else {
					System.err.println("Fracaso: El directorio respondió -> " + responseString);
					}
				} else {
					System.err.println("Fracaso: No se recibió respuesta del directorio.");
				}
	...
	public boolean pingDirectory() {
		...
		String msgText = requestMsg.toString();
		byte[] requestData = msgText.getBytes();
		byte[] responseData = sendAndReceiveDatagrams(requestData);
		if (responseData != null) {
			String responseString = new String(responseData);
			DirMessage responseMsg = DirMessage.fromString(responseString);
			String responseOp = responseMsg.getOperation();
			if (responseOp.equals("welcome")) {
				//System.out.println("Conexión con el directorio establecida (welcome).");
				success = true;
				} else {
					System.err.println("Fracaso : El directorio respondió con la operación -> " + responseOp);
					}
				} else {
					System.err.println("Fracaso : No se recibió respuesta del directorio.");
				}
	 ...
	 */
	    public boolean registerFileServer(String nickname, int serverPort, FileInfo[] files) {
		boolean success = false;

		// TODO: Ver TODOs en pingDirectory y seguir esquema similar
		
				DirMessage requestMsg = new DirMessage(DirMessageOps.OPERATION_REGISTER);
				requestMsg.setPort(serverPort);
				requestMsg.setFiles(files);
				requestMsg.setNickname(nickname);
				String msgText = requestMsg.toString();
				byte[] requestData = msgText.getBytes();
				byte[] responseData = sendAndReceiveDatagrams(requestData);
				...
				if (responseData != null) {
					String responseString = new String(responseData);
					DirMessage responseMsg = DirMessage.fromString(responseString);
					if (responseMsg.getOperation().equals(DirMessageOps.OPERATION_ACK)) {
					    System.out.println("* Nickname reassigned by directory: " + NanoFiles.UserNickname);
						success = true;
					} else {
						System.err.println("Fracaso : El directorio no aceptó el registro.");
					}
				}
	...
	public FileInfo[] getFileList() {
		...
		// TODO: Ver TODOs en pingDirectory y seguir esquema similar
		DirMessage requestMsg = new DirMessage(DirMessageOps.OPERATION_FILELIST);
		String msgText = requestMsg.toString();
		byte[] requestData = msgText.getBytes();
		byte[] responseData = sendAndReceiveDatagrams(requestData);
		if (responseData != null) {
			String responseString = new String(responseData);
			DirMessage responseMsg = DirMessage.fromString(responseString);
			if (responseMsg.getOperation().equals(DirMessageOps.OPERATION_FILELIST)) {
				filelist = responseMsg.getFiles();
				} else {
					System.err.println("Fracaso : El directorio no envió la lista de ficheros.");
					}
				}
	...
	public InetSocketAddress[] getServersSharingThisFile(String filenameSubstring) {
		...
		DirMessage requestMsg = new DirMessage(DirMessageOps.OPERATION_GETSERVERS);
		requestMsg.setFilename(filenameSubstring);
		String msgText = requestMsg.toString();
		byte[] requestData = msgText.getBytes();
		byte[] responseData = sendAndReceiveDatagrams(requestData);
		...
		if (responseData != null) {
			String responseString = new String(responseData);
			DirMessage responseMsg = DirMessage.fromString(responseString);
			if (responseMsg.getOperation().equals(DirMessageOps.OPERATION_GETSERVERS)) {
				serversList = responseMsg.getServerList();
				
				if (serversList == null) {
					serversList = new InetSocketAddress[0];
					System.err.println("Aviso: El directorio contestó, pero getServerList() devolvió null.");
				} else {
					System.out.println("Se han obtenido las direcciones de los servidores.");				}
			}
		}
	...
	public boolean unregisterFileServer() {
		boolean success = false;
		DirMessage requestMsg = new DirMessage(DirMessageOps.OPERATION_UNREGISTER);
		String msgText = requestMsg.toString();
		byte[] requestData = msgText.getBytes();
		byte[] responseData = sendAndReceiveDatagrams(requestData);
	...
	public InetSocketAddress[] getPeerList() {
		InetSocketAddress[] peerList = new InetSocketAddress[0];
		DirMessage requestMsg = new DirMessage(DirMessageOps.OPERATION_PEERLIST);
		String msgText = requestMsg.toString();
		byte[] requestData = msgText.getBytes();
		byte[] responseData = sendAndReceiveDatagrams(requestData);
		...
		if (responseData != null) {
			String responseString = new String(responseData);
			DirMessage responseMsg = DirMessage.fromString(responseString);
						if (responseMsg.getOperation().equals(DirMessageOps.OPERATION_PEERLIST)) {
							peerList = responseMsg.getServerList();
							String nombresStr = responseMsg.getNickname();
							if (nombresStr != null && peerList != null) {
								String[] nombres = nombresStr.split(",");
								System.out.println("\n=== PEERS REGISTRADOS ===");
								for (int i = 0; i < peerList.length; i++) {
									String nombrePeer = (i < nombres.length) ? nombres[i] : "Desconocido";
									System.out.println(" * Nickname: " + nombrePeer + 
													   " | IP: " + peerList[i].getAddress().getHostAddress() + 
													   " | Puerto TCP: " + peerList[i].getPort());
								}
							} 
						} else {
							System.err.println("Fracaso : El directorio no envió la lista de peers.");
						}
		}
	...
	public InetSocketAddress getPeerAddress(String nickname) {
		InetSocketAddress peerAddr = null;
		DirMessage requestMsg = new DirMessage(DirMessageOps.OPERATION_GETSERVERINFO);
		requestMsg.setNickname(nickname);
		String msgText = requestMsg.toString();
		byte[] requestData = msgText.getBytes();
		byte[] responseData = sendAndReceiveDatagrams(requestData);
		...
		if (responseData != null) {
			String responseString = new String(responseData);
			DirMessage responseMsg = DirMessage.fromString(responseString);
			if (responseMsg.getOperation().equals(DirMessageOps.OPERATION_GETSERVERINFO)) {
				InetSocketAddress[] servers = responseMsg.getServerList();
