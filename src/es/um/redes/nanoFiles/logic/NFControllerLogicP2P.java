...
public class NFControllerLogicP2P {
		...
		if (fileServer != null) {
			...
			try {
				...
				if (port > 0) {
					...
					System.out.println("NFServer server running on 0.0.0.0/0.0.0.0:" + port + ".");
					System.out.println("* You are now serving files on port: " + port);					
					serverRunning = true;
				} else {
					System.err.println("Error: El servidor no obtuvo un puerto válido.");
					fileServer = null;
				}
				
			} catch (IOException e) {
				System.err.println("Error de Entrada/Salida al intentar arrancar el servidor.");
				e.printStackTrace();
				fileServer = null;
			}
	 ...
	 *                                descargado
	 */
	protected boolean downloadFileFromServers(java.net.InetSocketAddress[] serverAddressList, String targetFileHash, String localFileName) {
		...
		for (java.net.InetSocketAddress serverAddr : serverAddressList) {
			try {
				System.out.println("Conectando con el peer remoto...");
				es.um.redes.nanoFiles.tcp.client.NFConnector connector = new es.um.redes.nanoFiles.tcp.client.NFConnector(serverAddr);
				java.io.File fileToSave = new java.io.File(localFileName);
				downloaded = connector.downloadFile(targetFileHash, fileToSave);
				connector.close();
				if (downloaded) {
					break;
				}
				...
				System.err.println("Falló la conexión con el servidor: " + serverAddr);
			}
		}
	...
	protected void stopFileServer() {
		...
		if (fileServer != null) {
			if (NanoFiles.directoryConnector != null) {
				NanoFiles.directoryConnector.unregisterFileServer();
			}
	...
	/**
	 * Método para descargar un fichero de un peer servidor de ficheros (Comando Básico 'peerdl')
	 */
	protected boolean downloadFileFromPeer(java.net.InetSocketAddress serverAddr, String targetFileHash, String localFileName) {
		boolean downloaded = false;
		...
		try {
			System.out.println("Conexión establecida con " + serverAddr.toString());
			es.um.redes.nanoFiles.tcp.client.NFConnector connector = new es.um.redes.nanoFiles.tcp.client.NFConnector(serverAddr);
			java.io.File fileToSave = new java.io.File(localFileName);
			downloaded = connector.downloadFile(targetFileHash, fileToSave);
			connector.close();
			...
			if (downloaded) {
				NanoFiles.db.addDownloadedFile(NanoFiles.sharedDirname);
				System.out.println("Fichero guardado correctamente como: " + localFileName);
			}
			
		} catch (Exception e) {
			System.err.println("Falló la conexión TCP directa con el servidor: " + serverAddr);
		}
