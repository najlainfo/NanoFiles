...
FileInfo[] trackedFiles = directoryConnector.getFileList(); 
		...
		if (trackedFiles != null && trackedFiles.length > 0) {
			...
			System.out.println("* The directory is currently tracking 0 files.");
		}
		return false;
	}
	/**
	...
	protected boolean registerFileServer(int serverPort, FileInfo[] filelist) {
		 ...
		 */
		boolean result = false;
		if (this.directoryConnector.registerFileServer(NanoFiles.UserNickname, serverPort, filelist)) {
	...
	public boolean getAndPrintPeerList() {
		System.out.println("* Requesting peer list from directory...");
		InetSocketAddress[] peerList = directoryConnector.getPeerList();
		if (peerList != null && peerList.length > 0) {
			System.out.println("* Active peers registered in the directory:");
			for (InetSocketAddress peer : peerList) {
				System.out.println("  - " + peer.toString());
			}
		} else {
			System.out.println("* No active peers found in the directory.");
		}
	...
	public InetSocketAddress getPeerAddress(String nickname) {
		return directoryConnector.getPeerAddress(nickname); 
	}
}
